...
type Ant = Point & {
  ...
  location: View; nestX: number; nestY: number; hunger: number; starving: number; dead: boolean;
};
type Spider = Point & {
...
function makeWorld() {
  ...
  const ants: Ant[] = Array.from({ length: 30 }, (_, index) => {
    ...
    return { kind:"ant", id:index+1, x:NEST.x+Math.cos(angle)*radius, y:NEST.y+Math.sin(angle)*radius,
      ...
      trailClock:0, aggro:0, location:"surface", nestX:800, nestY:110, hunger:random()*24, starving:0, dead:false };
  });
...
export default function Simulation() {
  ...
  useEffect(() => {
    ...
    const addStoredFood = (x=650+Math.random()*110, y=660+Math.random()*55) => world.storedFood.push({ id:world.nextFoodId++, x, y, taken:false });
    const enterNest = (ant: Ant) => { ant.location="nest"; ant.nestX=NEST_ENTRANCE.x+(Math.random()-.5)*12; ant.nestY=NEST_ENTRANCE.y+18; ant.target=null; ant.manual=false;
      if (ant.carrying && "radius" in ant.carrying) { addStoredFood(); ant.carrying=null; } if (ant===selectedRef.current) setView("nest"); syncHud(); };
    const leaveNest = (ant: Ant) => { ant.location="surface"; ant.x=NEST.x+(Math.random()-.5)*18; ant.y=NEST.y+(Math.random()-.5)*18; ant.target=null; ant.manual=false;
    ...
    const moveAnt = (ant: Ant, desired: number, dt: number, boost=1) => { ant.angle+=clamp(turnDifference(ant.angle,desired),-2.8*dt,2.8*dt); const speed=ant.speed*boost;
      ...
      else {ant.nestX+=Math.cos(ant.angle)*speed*dt;ant.nestY+=Math.sin(ant.angle)*speed*dt;ant.nestX=clamp(ant.nestX,45,W-45);ant.nestY=clamp(ant.nestY,72,H-42)} };
    ...
    const updateAnt = (ant: Ant, dt: number) => {
      ...
      if(ant.manual&&ant.target){const target=ant.target;desired=Math.atan2(target.y-position.y,target.x-position.x);if(distanceSquared(position,target)<110){
        ...
        if(ant.hunger>62){const food=nearestStoredFood(position);if(food){desired=Math.atan2(food.y-ant.nestY,food.x-ant.nestX);if(distanceSquared(position,food)<100){eatStoredFood(ant,food);return}}else desired=Math.atan2(NEST_ENTRANCE.y-ant.nestY,NEST_ENTRANCE.x-ant.nestX)}
        else if(ant.carrying){if("id" in ant.carrying){ant.carrying.x=ant.nestX;ant.carrying.y=ant.nestY;ant.carrying.taken=false}else addStoredFood(ant.nestX,ant.nestY);ant.carrying=null;syncHud()}
        else {desired=Math.atan2(NEST_ENTRANCE.y-ant.nestY,NEST_ENTRANCE.x-ant.nestX);if(distanceSquared(position,NEST_ENTRANCE)<18**2){leaveNest(ant);return}}
      }
    ...
    const growColony = (dt: number) => {world.growthClock-=dt;const stored=world.storedFood.filter(f=>!f.taken);if(world.growthClock<=0&&stored.length>=6&&activeAnts().length<60){stored[0].taken=true;stored[1].taken=true;const id=world.nextAntId++;world.ants.push({kind:"ant",id,x:NEST.x,y:NEST.y,angle:Math.PI/2,speed:45+Math.random()*10,carrying:null,target:null,manual:false,turn:(Math.random()-.5)*.6,trailClock:0,aggro:0,location:"nest",nestX:650+Math.random()*280,nestY:550+Math.random()*100,hunger:10,starving:0,dead:false});world.growthClock=28;notify("NEW WORKER · NEST EXPANDED");syncHud()}};
    ...
    const drawNest = () => {context.fillStyle="#332116";context.fillRect(0,0,W,H);context.fillStyle="#745035";context.fillRect(0,0,W,58);context.fillStyle="#20150f";context.fillRect(0,58,W,H-58);context.strokeStyle="#9a6841";context.lineWidth=36;context.lineCap="round";context.lineJoin="round";
      const paths=[[{x:800,y:55},{x:800,y:210},{x:610,y:310},{x:430,y:390}],[{x:800,y:210},{x:990,y:310},{x:1160,y:410}],[{x:610,y:310},{x:650,y:600}],[{x:990,y:310},{x:950,y:650}]];for(const path of paths){context.beginPath();context.moveTo(path[0].x,path[0].y);path.slice(1).forEach(p=>context.lineTo(p.x,p.y));context.stroke()}
      const chambers=[{x:430,y:390,rx:110,ry:65},{x:1160,y:410,rx:120,ry:70},{x:650,y:650,rx:135,ry:72},{x:950,y:680,rx:150,ry:82}];const extras=Math.max(0,Math.floor((activeAnts().length-30)/5));for(let i=0;i<extras;i++)chambers.push({x:250+(i%5)*265,y:790+Math.floor(i/5)*95,rx:82,ry:45});context.fillStyle="#9a6841";for(const chamber of chambers){context.beginPath();context.ellipse(chamber.x,chamber.y,chamber.rx,chamber.ry,0,0,TAU);context.fill()}
    ...
    const click = (event: MouseEvent) => {if(dragging)return;const point=pointer(event);if(viewRef.current==="surface"&&world.spider.alive&&distanceSquared(point,world.spider)<28**2){selectedRef.current=world.spider;world.spider.target=null;world.spider.manual=true;notify("SPIDER SELECTED");syncHud();return}const hit=hitAnt(point);if(hit){const choose=()=>{selectedRef.current=hit;hit.target=null;hit.manual=true;notify(hit.dead?`WORKER ${hit.id} · DECEASED`:`WORKER ${String(hit.id).padStart(2,"0")} SELECTED`);syncHud()};if(selectedRef.current?.kind==="spider"){clearTimeout(selectionTimer);selectionTimer=window.setTimeout(choose,220)}else choose();return}const selected=selectedRef.current;if(!selected)return;if(selected.kind==="spider"){if(viewRef.current!=="surface"||!selected.alive)return;selected.prey=null;selected.target=point;selected.manual=true;return}if(selected.dead||selected.location!==viewRef.current)return;if(selected.location==="surface"&&distanceSquared(point,NEST)<55**2){selected.target={...NEST_ENTRANCE,x:NEST.x,y:NEST.y,entrance:true};selected.manual=true;return}if(selected.location==="nest"&&distanceSquared(point,NEST_ENTRANCE)<55**2){selected.target={...NEST_ENTRANCE,entrance:true};selected.manual=true;return}selected.target={...point,drop:Boolean(selected.carrying)};selected.manual=true};
    const doubleClick = (event: MouseEvent) => {clearTimeout(selectionTimer);const point=pointer(event),selected=selectedRef.current;if(selected?.kind==="spider"&&viewRef.current==="surface"){const prey=hitAnt(point);if(prey&&!prey.dead){selected.prey=prey;selected.target=null;selected.manual=false;selected.pause=0;notify(`HUNTING WORKER ${String(prey.id).padStart(2,"0")}`)}return}if(selected?.kind!=="ant"||selected.dead||selected.location!==viewRef.current){notify("SELECT A LIVING ANT FIRST");return}if(viewRef.current==="surface"){const food=nearestSurfaceFood(point,20);if(food){selected.target={...food,food};selected.manual=true;notify("FOOD PICKUP ROUTE SET")}}else{const food=nearestStoredFood(point,20);if(food){selected.target={...food,stored:food};selected.manual=true;notify(selected.hunger>62?"FOOD EATING ROUTE SET":"FOOD MOVE ROUTE SET")}}};
    const pointerDown = (event: MouseEvent) => {if(viewRef.current!=="nest")return;const selected=selectedRef.current;if(selected?.kind!=="ant"||selected.dead||selected.location!=="nest")return;const point=pointer(event);if(distanceSquared(point,antPoint(selected))>80**2)return;dragging=true;activeDig={points:[{x:selected.nestX,y:selected.nestY},point]};world.digPaths.push(activeDig);selected.target={...point};selected.manual=true;notify("DIGGING TUNNEL")};
