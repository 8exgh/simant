...
export default function Simulation() {
  ...
  useEffect(() => {
    ...
    const rockLocal = (point:Point,rock:Rock) => {const cos=Math.cos(rock.rotation),sin=Math.sin(rock.rotation),dx=point.x-rock.x,dy=point.y-rock.y;return {x:dx*cos+dy*sin,y:-dx*sin+dy*cos}};
    const hitsRock = (point:Point,radius:number) => ROCKS.some(rock=>{const local=rockLocal(point,rock);return (local.x/(rock.rx+radius))**2+(local.y/(rock.ry+radius))**2<1});
    const resolveRocks = (point:Point,radius:number) => {for(const rock of ROCKS){const local=rockLocal(point,rock),ex=rock.rx+radius,ey=rock.ry+radius,depth=(local.x/ex)**2+(local.y/ey)**2;if(depth<1){const scale=depth>1e-6?1/Math.sqrt(depth):0,px=scale?local.x*scale:ex,py=scale?local.y*scale:0,cos=Math.cos(rock.rotation),sin=Math.sin(rock.rotation);point.x=rock.x+px*cos-py*sin;point.y=rock.y+px*sin+py*cos}}return point};
    const avoidRocks = (point:Point,angle:number,distance:number,radius:number) => {const blocked=(a:number)=>hitsRock({x:point.x+Math.cos(a)*distance,y:point.y+Math.sin(a)*distance},radius);if(!blocked(angle))return angle;for(const offset of [.7,-.7,1.4,-1.4,2.1,-2.1])if(!blocked(angle+offset))return angle+offset;return angle};
    ...
    const moveAnt = (ant: Ant, desired: number, dt: number, boost=1) => { if(ant.location==="surface")desired=avoidRocks(ant,desired,ant.speed*boost*dt+14,7); ant.angle+=clamp(turnDifference(ant.angle,desired),-2.8*dt,2.8*dt); const speed=ant.speed*boost;
      if(ant.location==="surface"){ant.x+=Math.cos(ant.angle)*speed*dt;ant.y+=Math.sin(ant.angle)*speed*dt;ant.x=clamp(ant.x,18,W-18);ant.y=clamp(ant.y,18,H-18);resolveRocks(ant,7)}
